...
describe('Tool Registry', () => {
  ...
  describe('initializeTools', () => {
    ...
    it('should register tools in correct categories', () => {
      ...
      const appTools = allTools.filter(tool => ['create_app', 'update_app', 'list_tables'].includes(tool.name));
      const tableTools = allTools.filter(tool => ['create_table', 'update_table', 'get_table_fields'].includes(tool.name));
      const fieldTools = allTools.filter(tool => ['create_field', 'update_field'].includes(tool.name));
      const recordTools = allTools.filter(tool => tool.name.includes('record'));
      const fileTools = allTools.filter(tool => tool.name.includes('file'));
      const reportTools = allTools.filter(tool => tool.name.includes('report'));
