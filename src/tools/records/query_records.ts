 ...
 */
export class QueryRecordsTool extends BaseTool<
  QueryRecordsParams,
  QueryRecordsResult
> {
  ...
  public paramSchema = {
    ...
    properties: {
      ...
      max_records: {
        type: "number",
        description:
          "Maximum number of records to return when paginating (default: 1000)",
      },
      skip: {
        type: "number",
    ...
    if (paginate && hasMore && allRecords.length < limit) {
      ...
      while (hasMore && allRecords.length < limit) {
        ...
        if (currentSkip <= previousSkip) {
          logger.error(
            "Pagination offset did not advance - potential infinite loop",
            {
              previousSkip,
              currentSkip,
