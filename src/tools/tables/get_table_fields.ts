 ...
 */
export class GetTableFieldsTool extends BaseTool<
  GetTableFieldsParams,
  GetTableFieldsResult
> {
   ...
   */
  protected async run(
    params: GetTableFieldsParams,
  ): Promise<GetTableFieldsResult> {
    ...
    if (field_type && field_type.trim() !== "") {
      fields = fields.filter(
        (field) => field.fieldType.toLowerCase() === field_type.toLowerCase(),
      );
    }
