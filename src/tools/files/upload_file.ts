...
> {
  ...
  protected async run(params: UploadFileParams): Promise<UploadFileResult> {
    ...
    if (fileInfo.size > MAX_FILE_SIZE) {
      throw new Error(
        `File size ${fileInfo.size} bytes exceeds maximum allowed size of ${MAX_FILE_SIZE} bytes`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Invalid file path: access outside working directory denied",
      );
    }
    ...
    try {
      ...
      throw new Error(
        `File access denied or file does not exist: ${file_path}`,
      );
    }
